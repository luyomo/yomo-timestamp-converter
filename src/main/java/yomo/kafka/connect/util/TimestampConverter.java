package yomo.kafka.connect.util;
...
//    private final SchemaBuilder datetimeSchema = SchemaBuilder.string().optional().name("yomo.time.DateTimeString");
        ...
        if (SUPPORTED_DATA_TYPES.stream().anyMatch(s -> s.equalsIgnoreCase(column.typeName()))) {
            ...
            registration.register(SchemaBuilder.string().optional(), rawValue -> {
                ...
                switch (column.typeName().toLowerCase()) {
                    case "time":
			String strTime = this.simpleTimeFormatter.format(dateObject);

		        String patternTime = "(.*).(\\d{6})";
		        Pattern rTime = Pattern.compile(patternTime);
                        Matcher mTime = rTime.matcher(strTime);
                        if (mTime.find()) {
                            String microseconds = this.getMicro(rawValue.toString());
                            if (microseconds != "" ){
			        String header = mTime.group(1);
                                return header + "." + microseconds;
                            }
			}
                        return strTime;
                    case "date":
                    ...
                    default:
                        String strDatetime = this.simpleDatetimeFormatter.format(dateObject);

		        String pattern = "(.*).(\\d{6})(.*)";
		        Pattern r = Pattern.compile(pattern);
                        Matcher m = r.matcher(strDatetime);
                        if (m.find()) {
                            String microseconds = this.getMicro(rawValue.toString());
                            if (microseconds != "" ){
			        String header = m.group(1);
                                String tail = m.group(3);
                                return header + "." + microseconds + tail;
                            }
			}
    ...
    private String getMicro(String timestamp) {
        String pattern = "(.*).(\\d{6})";
        Pattern r = Pattern.compile(pattern);
